...
class PythonRunner {
  ...
  async runCodeWithInput(code: string, input: string, timeout = 30000): Promise<PythonOutput> {
    const filepath = await this.createTempFile(code);
  ...
  async runInteractiveCode(code: string, inputs: string[], timeout = 30000): Promise<PythonOutput> {
    const filepath = await this.createTempFile(code);
...
const runCodeWithInputTool: Tool = {
  ...
  returnType: 'object',
  handler: async (params: any) => {
    return await pythonRunner.runCodeWithInput(params.code, params.input, params.timeout);
  },
};
...
const runInteractiveCodeTool: Tool = {
  ...
  returnType: 'object',
  handler: async (params: any) => {
    return await pythonRunner.runInteractiveCode(params.code, params.inputs, params.timeout);
  },
};

...
class TreeSitterManager {
  ...
  private formatNode(node: Parser.SyntaxNode, indent = ''): string {
    ...
    return nodeInfo.join('') + (children.length ? '\n' + children.join('\n') : '');
  }

  private collectNodeTypes(node: Parser.SyntaxNode, types = new Set<string>()): Set<string> {
    types.add(node.type);
    let cursor = node.walk();
    let hasChildren = cursor.gotoFirstChild();
  ...
  async queryCode(code: string, language: string, query: string): Promise<QueryResult> {
    try {
      const parser = this.languageParsers.get('.' + language.toLowerCase());
      if (!parser) {
        throw new Error(`Unsupported language: ${language}`);
  ...
  private async findFiles(directories: string[], patterns: string[]): Promise<string[]> {
...
const parseCodeTool: Tool = {
  ...
  parameters: [
    ...
    },
    {
      name: 'language',
      type: 'string',
      description: 'Programming language of the code (js, ts, py, rs, go, java, cpp, rb)',
      required: true,
    }
  ],
  returnType: 'object',
...
const queryCodeTool: Tool = {
  ...
  returnType: 'object',
  handler: async (params: any) => {
    return await treeSitterManager.queryCode(params.code, params.language, params.query);
  },
};
...
const searchCodebaseTool: Tool = {
  ...
  parameters: [
    ...
    },
    {
      name: 'filePatterns',
      type: 'array',
      description: 'Glob patterns for files to include (e.g., ["**/*.ts", "**/*.py"])',
      required: false,
    }
  ],
  returnType: 'array',

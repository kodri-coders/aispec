...
const DEFAULT_USER_AGENT = "ModelContextProtocol/1.0 (+https://github.com/modelcontextprotocol/servers)";

// Helper functions
async function getRobotsTxt(url: string, userAgent: string): Promise<string | null> {
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      throw new Error(`Robots.txt access forbidden (status ${response.status})`);
    }
...
async function fetchAndExtractContent(url: string, userAgent: string): Promise<string> {
  try {
